...
import it.eng.idra.beans.orion.OrionManagement;
...
public class OdmsSynchJob implements InterruptableJob {
  ...
  /** The enable Orion. */
  private static Boolean enableOrion = Boolean
      .parseBoolean(PropertyManager.getProperty(IdraProperty.ENABLE_ORION));
  
  /** The Orion URL. */  
  private static String urlOrion = 
      PropertyManager.getProperty(IdraProperty.ORION_URL);
  ...
  protected static boolean synchOdmsNode(OdmsCatalogue node, boolean isChangedProtocol)
    ...
    if (!node.isFederating()) {
      ...
      if (node.isOnline()) {
        ...
        // ----------------------- AGGIUNTA IL CATALOGO IN ORION ----------------------------

        //logger.info("Info: Passa di qui anche 
        //nella sincronizzazione oltre che nella federazione");
        OrionManagement om = new OrionManagement();
        om.addCatalogueInOrion(node);
